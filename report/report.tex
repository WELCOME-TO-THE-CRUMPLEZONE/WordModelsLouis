...
World Models is a particularly simple implementation of Schmidthuber's approach which has been shown to be remarkable effective at learning in image based environments. %CITEME
...
Alongside the original paper David Ha et al. released a browser-based version of these dreams which you can try out here. %CITEME
...
Since this output in turn depends on the output of the layer on the element before that (and so on) that the result is a sequential reccurence as depicted in Figure \ref{memFig}.
...
\begin{figure}[h]
  ...
  \end{tikzpicture}
\caption{An LSTM layer. The forget, input, and output gates are outlined in blue from left to right. Diagram modeled on } %CITEME
\end{figure}
...
Their stochastic output makes them great candidates for generative sequence prediction and they have seen a variety of applications, including in systems that complete human drawings or compose responses to human generated music. %CITEME
...
%begin in some initial state $z_0$, choose an action $a_0$ from a policy or human interaction and then sample $z_1$ from $p(z_{1} | z_0, a_0, h_0)$, and then continue iteratively.
...
\begin{figure}[h]
  ...
  \begin{minipage}{0.4\textwidth}
      ...
      \caption{CMA-ES: A narrow population resulting from a smaller covariance matrix. Credits: }%CITEME https://blog.otoro.net/2017/10/29/visual-evolution-strategies/
  \end{minipage}\hfill
  \begin{minipage}{0.4\textwidth}
      \centering
      \includegraphics[width=0.9\textwidth]{cmaes_step4.png} % second figure itself
      \caption{CMA-ES: A wider population resulting from a larger covariance matrix} %CITEME
  \end{minipage}
\end{figure}
...
I trained and tested these models in the \texttt{CarRacing-v0} environment as well as a \texttt{SimCity-Snes} environment that I built within OpenAI's gym retro system. %CITEME
...
Gym Retro is a project which turns old videogames into gym environments. %CITEME
